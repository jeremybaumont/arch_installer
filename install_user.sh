...
create-directories() {
    mkdir -p /home/"$(whoami)"/{Documents,Pictures,Downloads,Videos,workspace,Music,composer}
...
install-dotfiles() {
    ...
    if [ ! -d "$DOTFILES" ];
        then
            ...
            git clone --recurse-submodules "https://github.com/jeremybaumont/dotfiles" "$DOTFILES" >/dev/null
    fi
